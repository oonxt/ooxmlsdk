...
pub fn gen_open_xml_part(part: &OpenXmlPart, context: &GenContext) -> TokenStream {
        ...
        || part.name == "InternationalMacroSheetPart"
    {
        ...
        fields.push(quote! {
            pub root_element: crate::packages::opc_core_properties::CoreProperties,
        });
    ...
    for child in &part.children {
        ...
        if child.max_occurs_great_than_one {
            ...
            fields.push(quote! {
                pub #child_name_ident: #child_type,
            });
        } else {
            fields.push(quote! {
                pub #child_name_ident: Option<#child_type>,
            });
        }
    }
    ...
    if part.base == "OpenXmlPackage" {
        field_declaration_list.push(
            parse2(quote! {
                let content_types = crate::packages::opc_content_types::Types::from_str(&crate::common::read_string(&mut archive.by_name("[Content_Types].xml")?)?)?;
            })
                .unwrap(),
        );
        ...
        field_declaration_list.push(
            parse2(quote! {
                let part_target_str = if path.ends_with(".xml") {
                    &path[path.rfind('/').ok_or_else(|| crate::common::SdkError::CommonError(path.to_string()))?+ 1..path.len()]
                } else {
                    ""
                };
            })
        ...
        field_declaration_list.push(
            parse2(quote! {
                let relationships = if let Some(file_path) = file_path_set.get(&#part_rels_path_ident) {
                    ...
                    Some(crate::packages::opc_relationships::Relationships::from_str(&crate::common::read_string(&mut archive.by_name(file_path)?)?)?)
                } else {
                    None
                };
            })
        ...
        || part.name == "InternationalMacroSheetPart"
    {
        ...
        field_declaration_list.push(parse2(quote! {
            let root_element = Some(crate::packages::opc_core_properties::CoreProperties::from_str(&crate::common::read_string(&mut archive.by_name(path)?)?)?);
        }).unwrap());
        ...
        field_declaration_list.push(parse2(quote! {
            let root_element = Some(#field_type::from_str(&crate::common::read_string(&mut archive.by_name(path)?)?)?);
        }).unwrap());
    ...
    for child in &part.children {
        ...
        if child.max_occurs_great_than_one {
            ...
            field_declaration_list.push(
                parse2(quote! {
                    let mut #child_api_name_ident: Option<#child_type> = None;
                })
                    .unwrap(),
            );
            ...
            children_arm_list.push(
                parse2(quote! {
                    #relationship_type_ty => {
                        ...
                        #child_api_name_ident = Some(#child_type::new_from_archive(
                            &child_parent_path,
                            &target_path,
                            &relationship.id,
                            file_path_set,
