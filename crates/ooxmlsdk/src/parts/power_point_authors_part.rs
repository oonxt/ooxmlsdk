...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.microsoft.com/office/2018/10/relationships/authors";
...
impl PowerPointAuthorsPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_microsoft_com_office_powerpoint_2018_8_main::AuthorList::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let power_point_authors_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "."),
        );
        if !power_point_authors_part_dir_path.is_empty()
            && !entry_set.contains(&power_point_authors_part_dir_path)
        {
            zip.add_directory(
                &power_point_authors_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(power_point_authors_part_dir_path);
        }
