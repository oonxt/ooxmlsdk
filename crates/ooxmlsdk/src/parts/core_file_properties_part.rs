...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/package/2006/relationships/metadata/core-properties";
...
impl CoreFilePropertiesPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::packages::opc_core_properties::CoreProperties::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let core_file_properties_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "docProps"),
        );
        if !core_file_properties_part_dir_path.is_empty()
            && !entry_set.contains(&core_file_properties_part_dir_path)
        {
            zip.add_directory(
                &core_file_properties_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(core_file_properties_part_dir_path);
        }
