...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.microsoft.com/office/2011/relationships/timelineCache";
...
impl TimeLineCachePart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_microsoft_com_office_spreadsheetml_2010_11_main::TimelineCacheDefinition::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let time_line_cache_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "timelineCaches"),
        );
        if !time_line_cache_part_dir_path.is_empty()
            && !entry_set.contains(&time_line_cache_part_dir_path)
        {
            zip.add_directory(
                &time_line_cache_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(time_line_cache_part_dir_path);
        }
