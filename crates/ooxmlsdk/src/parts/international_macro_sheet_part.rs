...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.microsoft.com/office/2006/relationships/xlIntlMacrosheet";
...
impl InternationalMacroSheetPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let international_macro_sheet_part_rels_path = crate::common::resolve_zip_file_path(
            &format!("{}_rels/{}.rels", child_parent_path, part_target_str),
        );
        let relationships = if let Some(file_path) = file_path_set
            .get(&international_macro_sheet_part_rels_path)
        {
    ...
    pub(crate) fn save_zip(
        ...
        let international_macro_sheet_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "macrosheets"),
        );
        if !international_macro_sheet_part_dir_path.is_empty()
            && !entry_set.contains(&international_macro_sheet_part_dir_path)
        {
            zip.add_directory(
                &international_macro_sheet_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
