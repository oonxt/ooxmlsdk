...
pub struct SpreadsheetDocument {
    ...
    pub workbook_part: crate::parts::workbook_part::WorkbookPart,
...
impl SpreadsheetDocument {
    ...
    pub(crate) fn new_from_archive(
        ...
        let mut workbook_part: Option<crate::parts::workbook_part::WorkbookPart> = None;
        ...
        if let Some(relationships) = &relationships {
            for relationship in &relationships.relationship {
                ...
                match relationship.r#type.as_str() {
                    crate::parts::workbook_part::RELATIONSHIP_TYPE => {
                        let target_path = crate::common::resolve_zip_file_path(
                            &format!("{}{}", child_parent_path, relationship.target),
                        );
                        workbook_part = Some(
                            crate::parts::workbook_part::WorkbookPart::new_from_archive(
                                &child_parent_path,
                                &target_path,
                                &relationship.id,
                                file_path_set,
        ...
        let workbook_part = workbook_part
    ...
    pub(crate) fn save_zip(
        ...
        let spreadsheet_document_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, ""),
        );
        if !spreadsheet_document_dir_path.is_empty()
            && !entry_set.contains(&spreadsheet_document_dir_path)
        {
            zip.add_directory(
                &spreadsheet_document_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(spreadsheet_document_dir_path);
        }
        let child_parent_path = format!("{}{}", parent_path, "");
        ...
        self.workbook_part.save_zip(&child_parent_path, zip, entry_set)?;
