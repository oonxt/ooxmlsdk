...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/comments";
...
impl WordprocessingCommentsPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let wordprocessing_comments_part_rels_path = crate::common::resolve_zip_file_path(
            &format!("{}_rels/{}.rels", child_parent_path, part_target_str),
        );
        let relationships = if let Some(file_path) = file_path_set
            .get(&wordprocessing_comments_part_rels_path)
        {
        ...
        let root_element = Some(
            crate::schemas::schemas_openxmlformats_org_wordprocessingml_2006_main::Comments::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let wordprocessing_comments_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "."),
        );
        if !wordprocessing_comments_part_dir_path.is_empty()
            && !entry_set.contains(&wordprocessing_comments_part_dir_path)
        {
            zip.add_directory(
                &wordprocessing_comments_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(wordprocessing_comments_part_dir_path);
        }
