...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/xmlMaps";
...
impl CustomXmlMappingsPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_openxmlformats_org_spreadsheetml_2006_main::MapInfo::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let custom_xml_mappings_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "."),
        );
        if !custom_xml_mappings_part_dir_path.is_empty()
            && !entry_set.contains(&custom_xml_mappings_part_dir_path)
        {
            zip.add_directory(
                &custom_xml_mappings_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(custom_xml_mappings_part_dir_path);
        }
