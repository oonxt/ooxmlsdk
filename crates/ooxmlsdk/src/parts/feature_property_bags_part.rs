...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.microsoft.com/office/2022/11/relationships/FeaturePropertyBag";
...
impl FeaturePropertyBagsPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_microsoft_com_office_spreadsheetml_2022_featurepropertybag::FeaturePropertyBags::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let feature_property_bags_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "featurePropertyBag"),
        );
        if !feature_property_bags_part_dir_path.is_empty()
            && !entry_set.contains(&feature_property_bags_part_dir_path)
        {
            zip.add_directory(
                &feature_property_bags_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(feature_property_bags_part_dir_path);
        }
