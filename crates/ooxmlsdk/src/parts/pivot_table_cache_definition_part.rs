...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/pivotCacheDefinition";
...
pub struct PivotTableCacheDefinitionPart {
    ...
    pub root_element: crate::schemas::schemas_openxmlformats_org_spreadsheetml_2006_main::PivotCacheDefinition,
...
impl PivotTableCacheDefinitionPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let child_parent_path = format!("{}{}", parent_path, "../pivotCache/");
        ...
        let pivot_table_cache_definition_part_rels_path = crate::common::resolve_zip_file_path(
            &format!("{}_rels/{}.rels", child_parent_path, part_target_str),
        );
        let relationships = if let Some(file_path) = file_path_set
            .get(&pivot_table_cache_definition_part_rels_path)
        {
        ...
        let root_element = Some(
            crate::schemas::schemas_openxmlformats_org_spreadsheetml_2006_main::PivotCacheDefinition::from_str(
                &crate::common::read_string(&mut archive.by_name(path)?)?,
            )?,
        );
        let mut pivot_table_cache_records_part: Option<
            crate::parts::pivot_table_cache_records_part::PivotTableCacheRecordsPart,
        > = None;
        if let Some(relationships) = &relationships {
            for relationship in &relationships.relationship {
                ...
                match relationship.r#type.as_str() {
                    crate::parts::pivot_table_cache_records_part::RELATIONSHIP_TYPE => {
                        let target_path = crate::common::resolve_zip_file_path(
                            &format!("{}{}", child_parent_path, relationship.target),
                        );
                        pivot_table_cache_records_part = Some(
                            crate::parts::pivot_table_cache_records_part::PivotTableCacheRecordsPart::new_from_archive(
                                &child_parent_path,
                                &target_path,
                                &relationship.id,
                                file_path_set,
    ...
    pub(crate) fn save_zip(
        ...
        let pivot_table_cache_definition_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "../pivotCache"),
        );
        if !pivot_table_cache_definition_part_dir_path.is_empty()
            && !entry_set.contains(&pivot_table_cache_definition_part_dir_path)
        {
            zip.add_directory(
                &pivot_table_cache_definition_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(pivot_table_cache_definition_part_dir_path);
        }
        ...
        if let Some(pivot_table_cache_records_part) = &self
            .pivot_table_cache_records_part
        {
            pivot_table_cache_records_part.save_zip(&child_parent_path, zip, entry_set)?;
        }
        Ok(())
    }
}
