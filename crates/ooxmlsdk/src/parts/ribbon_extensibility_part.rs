...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.microsoft.com/office/2006/relationships/ui/extensibility";
...
pub struct RibbonExtensibilityPart {
    ...
    pub root_element: crate::schemas::schemas_microsoft_com_office_2006_01_customui::CustomUi,
...
impl RibbonExtensibilityPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let ribbon_extensibility_part_rels_path = crate::common::resolve_zip_file_path(
            &format!("{}_rels/{}.rels", child_parent_path, part_target_str),
        );
        let relationships = if let Some(file_path) = file_path_set
            .get(&ribbon_extensibility_part_rels_path)
        {
        ...
        let root_element = Some(
            crate::schemas::schemas_microsoft_com_office_2006_01_customui::CustomUi::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let ribbon_extensibility_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "customUI"),
        );
        if !ribbon_extensibility_part_dir_path.is_empty()
            && !entry_set.contains(&ribbon_extensibility_part_dir_path)
        {
            zip.add_directory(
                &ribbon_extensibility_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(ribbon_extensibility_part_dir_path);
        }
