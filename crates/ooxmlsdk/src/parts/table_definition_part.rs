...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/table";
...
impl TableDefinitionPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let child_parent_path = format!("{}{}", parent_path, "../tables/");
        ...
        let table_definition_part_rels_path = crate::common::resolve_zip_file_path(
            &format!("{}_rels/{}.rels", child_parent_path, part_target_str),
        );
        let relationships = if let Some(file_path) = file_path_set
            .get(&table_definition_part_rels_path)
        {
        ...
        let root_element = Some(
            crate::schemas::schemas_openxmlformats_org_spreadsheetml_2006_main::Table::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let table_definition_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "../tables"),
        );
        if !table_definition_part_dir_path.is_empty()
            && !entry_set.contains(&table_definition_part_dir_path)
        {
            zip.add_directory(
                &table_definition_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(table_definition_part_dir_path);
        }
