...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.microsoft.com/office/2006/relationships/attachedToolbars";
...
impl WordAttachedToolbarsPart {
    ...
    pub(crate) fn save_zip(
        ...
        let word_attached_toolbars_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "."),
        );
        if !word_attached_toolbars_part_dir_path.is_empty()
            && !entry_set.contains(&word_attached_toolbars_part_dir_path)
        {
            zip.add_directory(
                &word_attached_toolbars_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(word_attached_toolbars_part_dir_path);
        }
