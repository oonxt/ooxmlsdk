...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/themeOverride";
...
impl ThemeOverridePart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_openxmlformats_org_drawingml_2006_main::ThemeOverride::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let theme_override_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "theme"),
        );
        if !theme_override_part_dir_path.is_empty()
            && !entry_set.contains(&theme_override_part_dir_path)
        {
            zip.add_directory(
                &theme_override_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(theme_override_part_dir_path);
        }
