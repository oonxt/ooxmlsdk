...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/image";
...
impl ImagePart {
    ...
    pub(crate) fn save_zip(
        ...
        let image_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "../media"),
        );
        if !image_part_dir_path.is_empty() && !entry_set.contains(&image_part_dir_path) {
            zip.add_directory(
                &image_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(image_part_dir_path);
        }
        use std::io::Read;
