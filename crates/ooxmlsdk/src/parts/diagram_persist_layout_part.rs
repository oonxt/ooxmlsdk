...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.microsoft.com/office/2007/relationships/diagramDrawing";
...
impl DiagramPersistLayoutPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let child_parent_path = format!("{}{}", parent_path, "../diagrams/");
        ...
        let diagram_persist_layout_part_rels_path = crate::common::resolve_zip_file_path(
            &format!("{}_rels/{}.rels", child_parent_path, part_target_str),
        );
        let relationships = if let Some(file_path) = file_path_set
            .get(&diagram_persist_layout_part_rels_path)
        {
        ...
        let root_element = Some(
            crate::schemas::schemas_microsoft_com_office_drawing_2008_diagram::Drawing::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let diagram_persist_layout_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "../diagrams"),
        );
        if !diagram_persist_layout_part_dir_path.is_empty()
            && !entry_set.contains(&diagram_persist_layout_part_dir_path)
        {
            zip.add_directory(
                &diagram_persist_layout_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(diagram_persist_layout_part_dir_path);
        }
