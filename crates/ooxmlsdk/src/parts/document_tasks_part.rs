...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.microsoft.com/office/2019/05/relationships/documenttasks";
...
impl DocumentTasksPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_microsoft_com_office_tasks_2019_documenttasks::Tasks::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let document_tasks_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "."),
        );
        if !document_tasks_part_dir_path.is_empty()
            && !entry_set.contains(&document_tasks_part_dir_path)
        {
            zip.add_directory(
                &document_tasks_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(document_tasks_part_dir_path);
        }
