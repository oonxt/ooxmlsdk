...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/oleObject";
...
impl EmbeddedObjectPart {
    ...
    pub(crate) fn save_zip(
        ...
        let embedded_object_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "embeddings"),
        );
        if !embedded_object_part_dir_path.is_empty()
            && !entry_set.contains(&embedded_object_part_dir_path)
        {
            zip.add_directory(
                &embedded_object_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(embedded_object_part_dir_path);
        }
        use std::io::Read;
