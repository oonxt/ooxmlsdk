...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.microsoft.com/office/2006/relationships/activeXControlBinary";
...
impl EmbeddedControlPersistenceBinaryDataPart {
    ...
    pub(crate) fn save_zip(
        ...
        let embedded_control_persistence_binary_data_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "."),
        );
        if !embedded_control_persistence_binary_data_part_dir_path.is_empty()
            && !entry_set
                .contains(&embedded_control_persistence_binary_data_part_dir_path)
        {
            zip.add_directory(
                &embedded_control_persistence_binary_data_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(embedded_control_persistence_binary_data_part_dir_path);
        }
