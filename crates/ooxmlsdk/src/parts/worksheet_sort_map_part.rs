...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.microsoft.com/office/2006/relationships/wsSortMap";
...
impl WorksheetSortMapPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_microsoft_com_office_excel_2006_main::WorksheetSortMap::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let worksheet_sort_map_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "."),
        );
        if !worksheet_sort_map_part_dir_path.is_empty()
            && !entry_set.contains(&worksheet_sort_map_part_dir_path)
        {
            zip.add_directory(
                &worksheet_sort_map_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(worksheet_sort_map_part_dir_path);
        }
