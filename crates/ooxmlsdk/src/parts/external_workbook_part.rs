...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/externalLink";
#[derive(Clone, Debug, Default)]
pub struct ExternalWorkbookPart {
    pub r_id: String,
    pub inner_path: String,
    pub root_element: crate::schemas::schemas_openxmlformats_org_spreadsheetml_2006_main::ExternalLink,
}
impl ExternalWorkbookPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_openxmlformats_org_spreadsheetml_2006_main::ExternalLink::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let external_workbook_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "externalReferences"),
        );
        if !external_workbook_part_dir_path.is_empty()
            && !entry_set.contains(&external_workbook_part_dir_path)
        {
            zip.add_directory(
                &external_workbook_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(external_workbook_part_dir_path);
        }
