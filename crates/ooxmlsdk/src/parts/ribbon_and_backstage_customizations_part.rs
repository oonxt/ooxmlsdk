...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.microsoft.com/office/2007/relationships/ui/extensibility";
...
impl RibbonAndBackstageCustomizationsPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let child_parent_path = format!("{}{}", parent_path, "customUI/");
        ...
        let ribbon_and_backstage_customizations_part_rels_path = crate::common::resolve_zip_file_path(
            &format!("{}_rels/{}.rels", child_parent_path, part_target_str),
        );
        let relationships = if let Some(file_path) = file_path_set
            .get(&ribbon_and_backstage_customizations_part_rels_path)
        {
        ...
        let root_element = Some(
            crate::schemas::schemas_microsoft_com_office_2009_07_customui::CustomUi::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let ribbon_and_backstage_customizations_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "customUI"),
        );
        if !ribbon_and_backstage_customizations_part_dir_path.is_empty()
            && !entry_set.contains(&ribbon_and_backstage_customizations_part_dir_path)
        {
            zip.add_directory(
                &ribbon_and_backstage_customizations_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(ribbon_and_backstage_customizations_part_dir_path);
        }
