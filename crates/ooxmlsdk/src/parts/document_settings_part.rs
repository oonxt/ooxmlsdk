...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/settings";
...
impl DocumentSettingsPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let document_settings_part_rels_path = crate::common::resolve_zip_file_path(
            &format!("{}_rels/{}.rels", child_parent_path, part_target_str),
        );
        let relationships = if let Some(file_path) = file_path_set
            .get(&document_settings_part_rels_path)
        {
        ...
        let root_element = Some(
            crate::schemas::schemas_openxmlformats_org_wordprocessingml_2006_main::Settings::from_str(
                &crate::common::read_string(&mut archive.by_name(path)?)?,
            )?,
        );
        let mut mail_merge_recipient_data_part: Option<
            crate::parts::mail_merge_recipient_data_part::MailMergeRecipientDataPart,
        > = None;
        ...
        if let Some(relationships) = &relationships {
            for relationship in &relationships.relationship {
                ...
                match relationship.r#type.as_str() {
                    crate::parts::mail_merge_recipient_data_part::RELATIONSHIP_TYPE => {
                        let target_path = crate::common::resolve_zip_file_path(
                            &format!("{}{}", child_parent_path, relationship.target),
                        );
                        mail_merge_recipient_data_part = Some(
                            crate::parts::mail_merge_recipient_data_part::MailMergeRecipientDataPart::new_from_archive(
                                &child_parent_path,
                                &target_path,
                                &relationship.id,
                                file_path_set,
    ...
    pub(crate) fn save_zip(
        ...
        let document_settings_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "."),
        );
        if !document_settings_part_dir_path.is_empty()
            && !entry_set.contains(&document_settings_part_dir_path)
        {
            zip.add_directory(
                &document_settings_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(document_settings_part_dir_path);
        }
        ...
        if let Some(mail_merge_recipient_data_part) = &self
            .mail_merge_recipient_data_part
        {
            mail_merge_recipient_data_part.save_zip(&child_parent_path, zip, entry_set)?;
        }
