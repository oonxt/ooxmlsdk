...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/printerSettings";
...
impl WordprocessingPrinterSettingsPart {
    ...
    pub(crate) fn save_zip(
        ...
        let wordprocessing_printer_settings_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "../printerSettings"),
        );
        if !wordprocessing_printer_settings_part_dir_path.is_empty()
            && !entry_set.contains(&wordprocessing_printer_settings_part_dir_path)
        {
            zip.add_directory(
                &wordprocessing_printer_settings_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(wordprocessing_printer_settings_part_dir_path);
        }
