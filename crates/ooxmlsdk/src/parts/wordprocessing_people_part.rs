...
impl WordprocessingPeoplePart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_microsoft_com_office_word_2012_wordml::People::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let wordprocessing_people_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "."),
        );
        if !wordprocessing_people_part_dir_path.is_empty()
            && !entry_set.contains(&wordprocessing_people_part_dir_path)
        {
            zip.add_directory(
                &wordprocessing_people_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(wordprocessing_people_part_dir_path);
        }
