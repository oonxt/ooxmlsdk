...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/chartsheet";
...
impl ChartsheetPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let child_parent_path = format!("{}{}", parent_path, "chartsheets/");
        ...
        let root_element = Some(
            crate::schemas::schemas_openxmlformats_org_spreadsheetml_2006_main::Chartsheet::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let chartsheet_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "chartsheets"),
        );
        if !chartsheet_part_dir_path.is_empty()
            && !entry_set.contains(&chartsheet_part_dir_path)
        {
            zip.add_directory(
                &chartsheet_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(chartsheet_part_dir_path);
        }
