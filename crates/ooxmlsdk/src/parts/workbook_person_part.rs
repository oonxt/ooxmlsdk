...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.microsoft.com/office/2017/10/relationships/person";
#[derive(Clone, Debug, Default)]
pub struct WorkbookPersonPart {
    pub r_id: String,
    pub inner_path: String,
    pub root_element: crate::schemas::schemas_microsoft_com_office_spreadsheetml_2018_threadedcomments::PersonList,
}
impl WorkbookPersonPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_microsoft_com_office_spreadsheetml_2018_threadedcomments::PersonList::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let workbook_person_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "persons"),
        );
        if !workbook_person_part_dir_path.is_empty()
            && !entry_set.contains(&workbook_person_part_dir_path)
        {
            zip.add_directory(
                &workbook_person_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(workbook_person_part_dir_path);
        }
