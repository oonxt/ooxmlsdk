...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/numbering";
...
pub struct NumberingDefinitionsPart {
    ...
    pub root_element: crate::schemas::schemas_openxmlformats_org_wordprocessingml_2006_main::Numbering,
...
impl NumberingDefinitionsPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let numbering_definitions_part_rels_path = crate::common::resolve_zip_file_path(
            &format!("{}_rels/{}.rels", child_parent_path, part_target_str),
        );
        let relationships = if let Some(file_path) = file_path_set
            .get(&numbering_definitions_part_rels_path)
        {
        ...
        let root_element = Some(
            crate::schemas::schemas_openxmlformats_org_wordprocessingml_2006_main::Numbering::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let numbering_definitions_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "."),
        );
        if !numbering_definitions_part_dir_path.is_empty()
            && !entry_set.contains(&numbering_definitions_part_dir_path)
        {
            zip.add_directory(
                &numbering_definitions_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(numbering_definitions_part_dir_path);
        }
