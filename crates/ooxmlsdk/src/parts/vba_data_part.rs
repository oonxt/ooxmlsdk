...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.microsoft.com/office/2006/relationships/wordVbaData";
...
impl VbaDataPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_microsoft_com_office_word_2006_wordml::VbaSuppData::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let vba_data_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "."),
        );
        if !vba_data_part_dir_path.is_empty()
            && !entry_set.contains(&vba_data_part_dir_path)
        {
            zip.add_directory(
                &vba_data_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(vba_data_part_dir_path);
        }
