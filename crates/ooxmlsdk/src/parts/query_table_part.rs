...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/queryTable";
...
impl QueryTablePart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_openxmlformats_org_spreadsheetml_2006_main::QueryTable::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let query_table_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "../queryTables"),
        );
        if !query_table_part_dir_path.is_empty()
            && !entry_set.contains(&query_table_part_dir_path)
        {
            zip.add_directory(
                &query_table_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(query_table_part_dir_path);
        }
