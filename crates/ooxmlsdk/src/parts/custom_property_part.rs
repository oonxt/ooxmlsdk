...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/customProperty";
...
impl CustomPropertyPart {
    ...
    pub(crate) fn save_zip(
        ...
        let custom_property_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "."),
        );
        if !custom_property_part_dir_path.is_empty()
            && !entry_set.contains(&custom_property_part_dir_path)
        {
            zip.add_directory(
                &custom_property_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(custom_property_part_dir_path);
        }
        use std::io::Read;
