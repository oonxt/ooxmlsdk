...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.microsoft.com/office/2006/relationships/legacyDocTextInfo";
...
impl LegacyDiagramTextInfoPart {
    ...
    pub(crate) fn save_zip(
        ...
        let legacy_diagram_text_info_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "."),
        );
        if !legacy_diagram_text_info_part_dir_path.is_empty()
            && !entry_set.contains(&legacy_diagram_text_info_part_dir_path)
        {
            zip.add_directory(
                &legacy_diagram_text_info_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
