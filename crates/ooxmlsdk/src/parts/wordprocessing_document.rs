...
pub struct WordprocessingDocument {
    ...
    pub main_document_part: crate::parts::main_document_part::MainDocumentPart,
...
impl WordprocessingDocument {
    ...
    pub(crate) fn new_from_archive(
        ...
        let wordprocessing_document_rels_path = crate::common::resolve_zip_file_path(
            &format!("{}_rels/{}.rels", child_parent_path, part_target_str),
        );
        let relationships = if let Some(file_path) = file_path_set
            .get(&wordprocessing_document_rels_path)
        {
        ...
        let mut main_document_part: Option<
        ...
        if let Some(relationships) = &relationships {
            for relationship in &relationships.relationship {
                ...
                match relationship.r#type.as_str() {
                    crate::parts::main_document_part::RELATIONSHIP_TYPE => {
                        let target_path = crate::common::resolve_zip_file_path(
                            &format!("{}{}", child_parent_path, relationship.target),
                        );
                        main_document_part = Some(
                            crate::parts::main_document_part::MainDocumentPart::new_from_archive(
                                &child_parent_path,
                                &target_path,
                                &relationship.id,
                                file_path_set,
        ...
        let main_document_part = main_document_part
    ...
    pub(crate) fn save_zip(
        ...
        let wordprocessing_document_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, ""),
        );
        if !wordprocessing_document_dir_path.is_empty()
            && !entry_set.contains(&wordprocessing_document_dir_path)
        {
            zip.add_directory(
                &wordprocessing_document_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(wordprocessing_document_dir_path);
        }
        let child_parent_path = format!("{}{}", parent_path, "");
        ...
        self.main_document_part.save_zip(&child_parent_path, zip, entry_set)?;
