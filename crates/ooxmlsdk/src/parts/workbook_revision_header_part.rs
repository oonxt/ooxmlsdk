...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/revisionHeaders";
...
pub struct WorkbookRevisionHeaderPart {
    ...
    pub workbook_revision_log_parts: Vec<
...
impl WorkbookRevisionHeaderPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let child_parent_path = format!("{}{}", parent_path, "revisions/");
        ...
        let workbook_revision_header_part_rels_path = crate::common::resolve_zip_file_path(
            &format!("{}_rels/{}.rels", child_parent_path, part_target_str),
        );
        let relationships = if let Some(file_path) = file_path_set
            .get(&workbook_revision_header_part_rels_path)
        {
        ...
        let root_element = Some(
            crate::schemas::schemas_openxmlformats_org_spreadsheetml_2006_main::Headers::from_str(
                &crate::common::read_string(&mut archive.by_name(path)?)?,
            )?,
        );
        let mut workbook_revision_log_parts: Vec<
            crate::parts::workbook_revision_log_part::WorkbookRevisionLogPart,
        > = vec![];
        if let Some(relationships) = &relationships {
            for relationship in &relationships.relationship {
                ...
                match relationship.r#type.as_str() {
                    crate::parts::workbook_revision_log_part::RELATIONSHIP_TYPE => {
                        let target_path = crate::common::resolve_zip_file_path(
                            &format!("{}{}", child_parent_path, relationship.target),
                        );
                        let workbook_revision_log_part = crate::parts::workbook_revision_log_part::WorkbookRevisionLogPart::new_from_archive(
                            &child_parent_path,
                            &target_path,
                            &relationship.id,
                            file_path_set,
                        ...
                        workbook_revision_log_parts.push(workbook_revision_log_part);
                    }
                    _ => {}
                }
            }
    ...
    pub(crate) fn save_zip(
        ...
        let workbook_revision_header_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "revisions"),
        );
        if !workbook_revision_header_part_dir_path.is_empty()
            && !entry_set.contains(&workbook_revision_header_part_dir_path)
        {
            zip.add_directory(
                &workbook_revision_header_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(workbook_revision_header_part_dir_path);
        }
        ...
        for workbook_revision_log_part in &self.workbook_revision_log_parts {
            workbook_revision_log_part.save_zip(&child_parent_path, zip, entry_set)?;
        }
        Ok(())
    }
}
