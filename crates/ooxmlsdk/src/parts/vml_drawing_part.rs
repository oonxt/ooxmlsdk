...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/vmlDrawing";
...
impl VmlDrawingPart {
        ...
        if let Some(relationships) = &relationships {
            for relationship in &relationships.relationship {
                ...
                match relationship.r#type.as_str() {
                    ...
                    crate::parts::legacy_diagram_text_part::RELATIONSHIP_TYPE => {
                        let target_path = crate::common::resolve_zip_file_path(
                            &format!("{}{}", child_parent_path, relationship.target),
                        );
                        let legacy_diagram_text_part = crate::parts::legacy_diagram_text_part::LegacyDiagramTextPart::new_from_archive(
                            &child_parent_path,
                            &target_path,
                            &relationship.id,
                            file_path_set,
                        ...
                        legacy_diagram_text_parts.push(legacy_diagram_text_part);
                    }
                    _ => {}
                }
            }
    ...
    pub(crate) fn save_zip(
        ...
        let vml_drawing_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "../drawings"),
        );
        if !vml_drawing_part_dir_path.is_empty()
            && !entry_set.contains(&vml_drawing_part_dir_path)
        {
            zip.add_directory(
                &vml_drawing_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
        ...
        for legacy_diagram_text_part in &self.legacy_diagram_text_parts {
            legacy_diagram_text_part.save_zip(&child_parent_path, zip, entry_set)?;
        }
        Ok(())
    }
}
