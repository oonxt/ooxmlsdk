...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles";
...
impl StyleDefinitionsPart {
    ...
    pub(crate) fn save_zip(
        ...
        let style_definitions_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "."),
        );
        if !style_definitions_part_dir_path.is_empty()
            && !entry_set.contains(&style_definitions_part_dir_path)
        {
            zip.add_directory(
                &style_definitions_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(style_definitions_part_dir_path);
        }
