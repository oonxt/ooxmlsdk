...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.microsoft.com/office/2006/relationships/keyMapCustomizations";
...
pub struct CustomizationPart {
    ...
    pub root_element: crate::schemas::schemas_microsoft_com_office_word_2006_wordml::TemplateCommandGroup,
    pub word_attached_toolbars_part: Option<
...
impl CustomizationPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_microsoft_com_office_word_2006_wordml::TemplateCommandGroup::from_str(
                &crate::common::read_string(&mut archive.by_name(path)?)?,
            )?,
        );
        let mut word_attached_toolbars_part: Option<
            crate::parts::word_attached_toolbars_part::WordAttachedToolbarsPart,
        > = None;
        if let Some(relationships) = &relationships {
            for relationship in &relationships.relationship {
                ...
                match relationship.r#type.as_str() {
                    crate::parts::word_attached_toolbars_part::RELATIONSHIP_TYPE => {
                        let target_path = crate::common::resolve_zip_file_path(
                            &format!("{}{}", child_parent_path, relationship.target),
                        );
                        word_attached_toolbars_part = Some(
                            crate::parts::word_attached_toolbars_part::WordAttachedToolbarsPart::new_from_archive(
                                &child_parent_path,
                                &target_path,
                                &relationship.id,
                                file_path_set,
    ...
    pub(crate) fn save_zip(
        ...
        let customization_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "."),
        );
        if !customization_part_dir_path.is_empty()
            && !entry_set.contains(&customization_part_dir_path)
        {
            zip.add_directory(
                &customization_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(customization_part_dir_path);
        }
        ...
        if let Some(word_attached_toolbars_part) = &self.word_attached_toolbars_part {
            word_attached_toolbars_part.save_zip(&child_parent_path, zip, entry_set)?;
        }
        Ok(())
    }
}
