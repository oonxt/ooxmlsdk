...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.microsoft.com/office/2017/10/relationships/threadedComment";
...
impl WorksheetThreadedCommentsPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_microsoft_com_office_spreadsheetml_2018_threadedcomments::ThreadedComments::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let worksheet_threaded_comments_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "../threadedcomments"),
        );
        if !worksheet_threaded_comments_part_dir_path.is_empty()
            && !entry_set.contains(&worksheet_threaded_comments_part_dir_path)
        {
            zip.add_directory(
                &worksheet_threaded_comments_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(worksheet_threaded_comments_part_dir_path);
        }
