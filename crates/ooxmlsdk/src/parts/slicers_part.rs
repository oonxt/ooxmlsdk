...
impl SlicersPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_microsoft_com_office_spreadsheetml_2009_9_main::Slicers::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let slicers_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "../slicers"),
        );
        if !slicers_part_dir_path.is_empty()
            && !entry_set.contains(&slicers_part_dir_path)
        {
            zip.add_directory(
                &slicers_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(slicers_part_dir_path);
        }
