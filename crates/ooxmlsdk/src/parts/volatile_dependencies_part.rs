...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/volatileDependencies";
#[derive(Clone, Debug, Default)]
pub struct VolatileDependenciesPart {
    pub r_id: String,
    pub inner_path: String,
    pub root_element: crate::schemas::schemas_openxmlformats_org_spreadsheetml_2006_main::VolatileTypes,
}
impl VolatileDependenciesPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_openxmlformats_org_spreadsheetml_2006_main::VolatileTypes::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let volatile_dependencies_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "."),
        );
        if !volatile_dependencies_part_dir_path.is_empty()
            && !entry_set.contains(&volatile_dependencies_part_dir_path)
        {
            zip.add_directory(
                &volatile_dependencies_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(volatile_dependencies_part_dir_path);
        }
