...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/connections";
...
impl ConnectionsPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_openxmlformats_org_spreadsheetml_2006_main::Connections::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let connections_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "."),
        );
        if !connections_part_dir_path.is_empty()
            && !entry_set.contains(&connections_part_dir_path)
        {
            zip.add_directory(
                &connections_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(connections_part_dir_path);
        }
