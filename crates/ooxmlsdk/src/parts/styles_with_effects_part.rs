...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.microsoft.com/office/2007/relationships/stylesWithEffects";
...
impl StylesWithEffectsPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_openxmlformats_org_wordprocessingml_2006_main::Styles::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let styles_with_effects_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "."),
        );
        if !styles_with_effects_part_dir_path.is_empty()
            && !entry_set.contains(&styles_with_effects_part_dir_path)
        {
            zip.add_directory(
                &styles_with_effects_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(styles_with_effects_part_dir_path);
        }
