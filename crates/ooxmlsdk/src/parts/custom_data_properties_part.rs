...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.microsoft.com/office/2007/relationships/customDataProps";
...
pub struct CustomDataPropertiesPart {
    ...
    pub root_element: crate::schemas::schemas_microsoft_com_office_spreadsheetml_2009_9_main::DatastoreItem,
    pub custom_data_part: Option<crate::parts::custom_data_part::CustomDataPart>,
}
impl CustomDataPropertiesPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let child_parent_path = format!("{}{}", parent_path, "customData/");
        ...
        let custom_data_properties_part_rels_path = crate::common::resolve_zip_file_path(
            &format!("{}_rels/{}.rels", child_parent_path, part_target_str),
        );
        let relationships = if let Some(file_path) = file_path_set
            .get(&custom_data_properties_part_rels_path)
        {
        ...
        let root_element = Some(
            crate::schemas::schemas_microsoft_com_office_spreadsheetml_2009_9_main::DatastoreItem::from_str(
                &crate::common::read_string(&mut archive.by_name(path)?)?,
            )?,
        );
        let mut custom_data_part: Option<
        ...
        if let Some(relationships) = &relationships {
            for relationship in &relationships.relationship {
                ...
                match relationship.r#type.as_str() {
                    crate::parts::custom_data_part::RELATIONSHIP_TYPE => {
                        let target_path = crate::common::resolve_zip_file_path(
                            &format!("{}{}", child_parent_path, relationship.target),
                        );
                        custom_data_part = Some(
                            crate::parts::custom_data_part::CustomDataPart::new_from_archive(
                                &child_parent_path,
                                &target_path,
                                &relationship.id,
                                file_path_set,
    ...
    pub(crate) fn save_zip(
        ...
        let custom_data_properties_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "customData"),
        );
        if !custom_data_properties_part_dir_path.is_empty()
            && !entry_set.contains(&custom_data_properties_part_dir_path)
        {
            zip.add_directory(
                &custom_data_properties_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(custom_data_properties_part_dir_path);
        }
        ...
        if let Some(custom_data_part) = &self.custom_data_part {
            custom_data_part.save_zip(&child_parent_path, zip, entry_set)?;
        }
        Ok(())
    }
}
