...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/sheetMetadata";
...
impl CellMetadataPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_openxmlformats_org_spreadsheetml_2006_main::Metadata::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let cell_metadata_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "."),
        );
        if !cell_metadata_part_dir_path.is_empty()
            && !entry_set.contains(&cell_metadata_part_dir_path)
        {
            zip.add_directory(
                &cell_metadata_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(cell_metadata_part_dir_path);
        }
