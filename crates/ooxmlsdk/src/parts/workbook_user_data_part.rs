...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/usernames";
...
impl WorkbookUserDataPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_openxmlformats_org_spreadsheetml_2006_main::Users::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let workbook_user_data_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "revisions"),
        );
        if !workbook_user_data_part_dir_path.is_empty()
            && !entry_set.contains(&workbook_user_data_part_dir_path)
        {
            zip.add_directory(
                &workbook_user_data_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(workbook_user_data_part_dir_path);
        }
