...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.microsoft.com/office/2020/07/relationships/rdrichvaluewebimage";
...
impl RdRichValueWebImagePart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_microsoft_com_office_spreadsheetml_2020_richdatawebimage::WebImagesSupportingRichData::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let rd_rich_value_web_image_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "richData"),
        );
        if !rd_rich_value_web_image_part_dir_path.is_empty()
            && !entry_set.contains(&rd_rich_value_web_image_part_dir_path)
        {
            zip.add_directory(
                &rd_rich_value_web_image_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(rd_rich_value_web_image_part_dir_path);
        }
