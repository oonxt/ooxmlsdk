...
impl ExcelAttachedToolbarsPart {
    ...
    pub(crate) fn save_zip(
        ...
        let excel_attached_toolbars_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "."),
        );
        if !excel_attached_toolbars_part_dir_path.is_empty()
            && !entry_set.contains(&excel_attached_toolbars_part_dir_path)
        {
            zip.add_directory(
                &excel_attached_toolbars_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
