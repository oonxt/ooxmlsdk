...
impl WorksheetCommentsPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_openxmlformats_org_spreadsheetml_2006_main::Comments::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let worksheet_comments_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, ".."),
        );
        if !worksheet_comments_part_dir_path.is_empty()
            && !entry_set.contains(&worksheet_comments_part_dir_path)
        {
            zip.add_directory(
                &worksheet_comments_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(worksheet_comments_part_dir_path);
        }
