...
pub const RELATIONSHIP_TYPE: &str = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/webSettings";
#[derive(Clone, Debug, Default)]
pub struct WebSettingsPart {
    pub r_id: String,
    pub inner_path: String,
    pub root_element: crate::schemas::schemas_openxmlformats_org_wordprocessingml_2006_main::WebSettings,
}
impl WebSettingsPart {
    ...
    pub(crate) fn new_from_archive(
        ...
        let root_element = Some(
            crate::schemas::schemas_openxmlformats_org_wordprocessingml_2006_main::WebSettings::from_str(
    ...
    pub(crate) fn save_zip(
        ...
        let web_settings_part_dir_path = crate::common::resolve_zip_file_path(
            &format!("{}{}/", parent_path, "."),
        );
        if !web_settings_part_dir_path.is_empty()
            && !entry_set.contains(&web_settings_part_dir_path)
        {
            zip.add_directory(
                &web_settings_part_dir_path,
                zip::write::SimpleFileOptions::default(),
            )?;
            entry_set.insert(web_settings_part_dir_path);
        }
